...
public class FileRenameHandler : CommandHandlerBase
{
    public override ICommand? Handle(IList<string> commands)
    {
        if (commands.Count == 4 && commands[0] == "file" && commands[1] == "rename")
        {
            return new FileRenameCommand(commands[2], commands[3]);
        }
